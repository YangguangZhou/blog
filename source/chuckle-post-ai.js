...
function ChucklePostAI(AI_option) {
  ...
  function MAIN(AI_option) {
    ...
    const currentURL = "https://blog.jerryz.com.cn" + currentPath;
    // 排除页面
    ...
    const interface = {
      name: "JZ-AI",
      introduce: "我是文章辅助AI: JZ-AI，点击下方的按钮，让我生成本文简介、推荐相关文章等。",
      version: "GPT-4",
    ...
    async function recommendList() {
      ...
      if(sessionStorage.getItem('recommendList')){
        ...
        try {
          response = await fetch(`https://blog-ai.jerryz.com.cn/recommends?url=${encodeURIComponent(window.location.href)}&author=${AI_option.rec_method ? AI_option.rec_method : 'all'}`, options);
    ...
    async function matrixShuttle(){
      ...
      if(sessionStorage.getItem('matrixShuttle')){
        ...
        try {
          response = await fetch('https://blog-ai.jerryz.com.cn/websites_used', options);
          completeGenerate = true;
          if (response.status === 429) {
            startAI('请求过于频繁，请稍后再请求AI。');
          }
    ...
    async function summarySpeech(){
      ...
      if(summary_audio && summary_audio){
        ...
        try {
          ai_speech.style.pointerEvents = "none";
          ai_speech.style.opacity = "0.4";
          response = await fetch(`https://blog-ai.jerryz.com.cn/audio?${requestParams}`, options);
    ...
    async function getGptResponse(content, i = true) {
      ...
      if (i) {
        try {
          response = await fetch('https://blog-ai.jerryz.com.cn/', {
            signal: signal,
            method: "POST",
            headers: {
              "Content-Type": "application/json",
